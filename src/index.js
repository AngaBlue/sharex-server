...
class Server {
     ...
     * @param {number} config.host The hostname the server will use
     * @param {number} config.fileLength The desired length of the file name
     */
    constructor(config = {
        password: "1234", path: "uploads", port: 6060, fileLength: 10,
    }) {
    ...
    _routes() {
        ...
        this._server.get(":img", async (req, res) => {
        ...
        this._server.post("/api/upload", async (req, res) => {
            ...
            // Generate a string
            const string = nanoid(this._fileLength || 10);
